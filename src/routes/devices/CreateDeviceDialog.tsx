...
export function CreateDeviceDialog({ open, onClose }: CreateDeviceDialogProps) {
  const { addDevice, isLoading, error } = useDevicesStore();
  const {
    tenants,
  ...
  const validateForm = (): boolean => {
    try {
      ...
      const dataToValidate: CreateDevice = {
      ...
      CreateDeviceSchema.parse(dataToValidate);
      setValidationErrors({});
      setSubmitError(null);
      return true;
    } catch (error) {
  ...
  // console.log('Zod Data:', CreateDeviceSchema.safeParse(formData));
  ...
  return (
    ...
    >
      ...
      <DialogActions sx={{ p: 3, gap: 1 }}>
        ...
        >
          {isLoading ? 'Criando...' : 'Criar Dispositivo'}
        </Button>
      </DialogActions>
    </Dialog>
  );
