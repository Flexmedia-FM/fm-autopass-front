...
import { AtmStatusLabels, CreateAtmSchema, type CreateAtm } from './schema';

interface CreateAtmDialogProps {
  open: boolean;
  onClose: () => void;
}
...
export function CreateAtmDialog({ open, onClose }: CreateAtmDialogProps) {
  const [formData, setFormData] = useState<FormData>(initialFormData);
  const [validationErrors, setValidationErrors] = useState<
    Record<string, string>
  >({});
  const [submitError, setSubmitError] = useState<string | null>(null);
  const { addAtm, isLoading, error } = useAtmsStore();
  const {
    tenants,
  ...
  const {
    ...
    autoLoad: false,
    tenantId: formData.tenantId || initialFormData.tenantId,
  });
  ...
  useEffect(() => {
    console.log('Loading installations for tenant:', formData.tenantId);
  ...
  const validateField = (field: keyof FormData, value: string | boolean) => {
    try {
      const fieldSchema = CreateAtmSchema.shape[field as keyof CreateAtm];
      if (fieldSchema) {
  ...
  const validateForm = (): boolean => {
    try {
      const atmData: CreateAtm = {
        ...
        deviceStatus: formData.status as 'ACTIVE' | 'INACTIVE' | 'MAINTENANCE',
        // isActive: formData.isActive,
        notes: formData.notes || undefined,
      };

      CreateAtmSchema.parse(atmData);
      setValidationErrors({});
      return true;
    } catch (error: unknown) {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>Adicionar Novo ATM</DialogTitle>
...
export default CreateAtmDialog;
